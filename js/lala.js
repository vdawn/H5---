...
function wxShare(wxShareTitle, wxShareSummary, wxFriendShareStr, targetUrl, wxSharePicUrl, wxShareFriendsCirclePicUrl) {
    ...
    //格式化分享文字内容--去除文字中的HTML代码
    wxShareTitle = (typeof getTextFromHtml != 'undefined') && getTextFromHtml(wxShareTitle);
    wxShareSummary = (typeof getTextFromHtml != 'undefined') && getTextFromHtml(wxShareSummary);
    wxFriendShareStr = (typeof getTextFromHtml != 'undefined') && getTextFromHtml(wxFriendShareStr);
    ...
    // var wxSharePicUrl = hostConf + "/themes/img/wenfang.jpg?v=20170526154826";
    ...
    } else if (targetUrlPathname == "/qanda/qanda_question.html") {
        wxShareFromUrl(wxShareTitle, wxShareSummary, wxFriendShareStr, "", wxSharePicUrl, wxShareFriendsCirclePicUrl);
            ...
            if (result.result == "success") {
                wx.config({
                    ...
                    jsApiList: [
                        ...
                        'stopVoice',
                        'uploadVoice',
                        'downloadVoice', 'checkJsApi', 'chooseImage', 'previewImage', 'uploadImage', 'downloadImage', 'getLocalImgData'
                    ]
                });
...
function appShareFunction() {
    if (typeof wxShareminiApps != 'undefined' && typeof wxAPPId != 'undefined' && wxShareminiApps.isMiniProgram === 1 && (initOs.getOs() == 'ios' || initOs.getOs() == 'android' && initOs.compareVersion("2.1.2"))) {
        cordova.exec(callAppsSuccessFunction, callAppsFailFunction, "MyPlugin", "share", [appShareData.wxShareTitle, appShareData.wxShareSummary, appShareData.wxSharePicUrl, appShareData.targetUrl, wxAPPId, wxShareminiApps.path, wxShareminiApps.isMiniProgram]);
    } else {
        appShareData.wxSharePicUrl = appShareData.wxSharePicUrl.replace(/appType=isApp&/, '').replace(/appVersions=([^&])+(&|$)/, '');
        cordova.exec(callAppsSuccessFunction, callAppsFailFunction, "MyPlugin", "share", [appShareData.wxShareTitle, appShareData.wxShareSummary, appShareData.wxFriendShareStr, appShareData.wxSharePicUrl, appShareData.wxShareFriendsCirclePicUrl, appShareData.targetUrl, 0]);
    }
}